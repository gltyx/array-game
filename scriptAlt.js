...
function reset() {
  game = {
    ...
    BUpgradesBought: [new Decimal(0), new Decimal(0), new Decimal(0), new Decimal(0), new Decimal(0), new Decimal(0), new Decimal(0), new Decimal(0), new Decimal(0)],
		BUpgradeCosts: [new Decimal(1e90)],
...
function loadGame(loadgame) {
  ...
  if (game.unlocks >= 2) {
    ...
    $(".tabButton").eq(3).css("display", "block")
  ...
  if (game.currentTab == 0) {
    ...
    for (i = 0; i < 5; i++) {
      ...
      if (game.AGeneratorsBought[i] > 20) {
        if (game.BUpgradesBought[1].eq(1)) {
    ...
    $("#CBoost").html(format(game.array[2].pow(0.6).mul(3).add(1), 2));
      ...
      if (game.CGeneratorsBought[i] > 20) {
        $(".CGeneratorMultiplier").eq(i).html(" (*" + format(game.CGeneratorsBought[i].sub(19).pow(0.2), 2) + ")")
      }
      else {
...
function updateVariables() {
  ...
  for (i = 0; i < 5; i++) {
    ...
    if (game.AGeneratorsBought[i] > 20) {
      if (game.BUpgradesBought[1].eq(1)) {game.AGeneratorProduction[i] = game.AGeneratorProduction[i].mul(game.AGeneratorsBought[i].sub(19).pow(0.6))}
  ...
  if (game.currentChallenge == 1) game.AGeneratorProduction[0] = game.AGeneratorProduction[0].pow(0.5)
  ...
  game.AGenerators[3] = game.AGenerators[3].add(game.AGeneratorProduction[4].div(timeDivider))
	if (game.CMilestonesReached >= 6) buyUpgrade(1,4)
	if (game.CMilestonesReached >= 8 && game.currentChallenge != 1) buyMaxGenerators(1, 6)
  ...
  if (game.unlocks >= 2) game.BToGet = game.BToGet.mul(game.array[2].pow(0.6).mul(3).add(1))
	if (game.CMilestonesReached >= 5) game.BToGet = game.BToGet.mul(game.BGenerators[4].add(1).pow(game.challengesBeaten[1] / 1.5).floor())
  ...
  if (game.unlocks >= 1) {
    ...
    game.AGenerators[4] = game.AGenerators[4].add(game.BGeneratorProduction[0].div(timeDivider))
    game.BGenerators[0] = game.BGenerators[0].add(game.BGeneratorProduction[1].div(timeDivider))
    game.BGenerators[1] = game.BGenerators[1].add(game.BGeneratorProduction[2].div(timeDivider))
    game.BGenerators[2] = game.BGenerators[2].add(game.BGeneratorProduction[3].div(timeDivider))
    game.BGenerators[3] = game.BGenerators[3].add(game.BGeneratorProduction[4].div(timeDivider))
    ...
    $("#nextC").html(format(new Decimal(10).pow(game.CToGet.add(1).pow(0.5).mul(10))))
		...
		if (game.CMilestonesReached >= 14) game.CToGet = game.CToGet.mul(new Decimal(3).pow(game.BUpgradesBought[8]))
    ...
    $("#CToGet").html(format(game.CToGet))
		...
		if (game.unlocks >= 2) {
			...
			for (i = 0; i < 5; i++) {
				...
				if (game.CGeneratorsBought[i] > 20) game.CGeneratorProduction[i] = game.CGeneratorProduction[i].mul(game.CGeneratorsBought[i].sub(19).pow(0.2))
			}
			game.BGenerators[4] = game.BGenerators[4].add(game.CGeneratorProduction[0].div(timeDivider))
			...
			if (game.CMilestonesReached >= 10) {
				...
				if (game.CMilestonesReached >= 11 && document.getElementsByClassName("challenge")[3].style.display != "inline-block") $(".challenge").eq(3).css("display", "inline-block")
				if (game.CMilestonesReached >= 14 && document.getElementsByClassName("BUpgrade")[8].style.display != "inline-block") {
...
function buyMaxABoosterators() {
	...
	game.array[1] = game.array[1].sub(boosteratorCost)
